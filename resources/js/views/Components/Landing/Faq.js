...
export default function Faq({id}) {
  ...
  const Result1 = () => (
    <div>
      <p style={{ fontSize: "16px" }}>AIによる自動処理を活用する事で、価格と品質を両立させています。また、納品前には国内で品質管理を行っています。
      </p>
    </div>
  )
  const Result2 = () => (
    <div>
      <p style={{ fontSize: "16px" }}>最終的な成果物の精度は、お客様からご提供頂く依頼データの精度に依存します。
      </p>
    </div>
  )
  const Result3 = () => (
    <div>
      <p style={{ fontSize: "16px" }}>価格と納期はご依頼内容によって異なります。詳しくはログイン後の、案件作成画面でご確認頂けます。
      </p>
    </div>
  )
  const Result4 = () => (
    <div>
      <p style={{ fontSize: "16px" }}>精度10cm以下のグラウンドデータとオルソフォトが必要になります。また、等高線があると品質が向上します。
      </p>
    </div>
  )
  const Result5 = () => (
    <div>
      <p style={{ fontSize: "16px" }}>ご提供頂くグラウンドデータとオルソ画像から読み取れる地物のみを一般図として図化します。
      </p>
    </div>
  )
  const Result6 = () => (
    <div>
      <p style={{ fontSize: "16px" }}>写真測量・Lidar測量いずれも対応可能です。お見積りが必要な場合は、ページ下部のお問合せフォームよりお気軽にご相談下さい。
      </p>
    </div>
  )
  const Result7 = () => (
    <div>
      <p style={{ fontSize: "16px" }}>ご利用の際にはGoogle Chromeをご使用下さい。それ以外のブラウザによっては、正しくご利用できない場合があります。
      </p>
    </div>
  )
